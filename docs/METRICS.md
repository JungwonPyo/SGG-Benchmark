...
### F1 Score@K (F1@K)
It is proposed by [Fine-Grained Scene Graph Generation with Data Transfer](https://arxiv.org/abs/2203.11654) to balance the trade-off between Recall@K and mean Recall@K. The F1 Score is the harmonic mean of Recall and meanRecall, which is calculated as follows:
F1@K = 2 * R@K * mR@K / (R@K + mR@K)
This metric efficiently represent the trade-off between performance on head claases (Recall@K) and tail classes (mean Recall@K). A high F1 Score indicates that the model performs well on both head and tail classes, while a low F1 Score indicates that the model is biased towards head classes or tail classes. It is the best metric to monitor for real world deployment.
