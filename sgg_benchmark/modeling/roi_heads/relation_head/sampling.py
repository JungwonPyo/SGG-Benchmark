...
from sgg_benchmark.structures.box_ops import box_iou, box_convert
...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals):
        ...
        for p in proposals:
            ...
            if (not self.use_gt_box) and self.test_overlap:
                prp_boxes = box_convert(p["boxes"], p["mode"], "xyxy")
                cand_matrix = (cand_matrix.byte().bool() & box_iou(prp_boxes, prp_boxes).gt(0).byte().bool())
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            assert proposal["boxes"].shape[0] == target["boxes"].shape[0], \
                "Proposals shape: {} vs Targets shape: {}".format(
                    proposal["boxes"].shape[0], target["boxes"].shape[0])
            tgt_rel_matrix = target["relation"].to(device)  # [tgt, tgt]
    ...
    def detect_relsample(self, proposals, targets):
        ...
        Arguments:
            proposals (list[Dict])  contain fields: labels, predict_logits
            targets (list[Dict]) contain fields: labels
        """
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            device = proposal["boxes"].device
            prp_boxes = box_convert(proposal["boxes"], proposal["mode"], "xyxy")
            prp_lab = proposal["labels"].long()
            tgt_boxes = box_convert(target["boxes"], target["mode"], "xyxy").to(device)
            tgt_lab = target["labels"].long().to(device)
            tgt_rel_matrix = target["relation"].to(device)  # [tgt, tgt]

            # IoU matching
            ious = box_iou(tgt_boxes, prp_boxes)  # [tgt, prp]
            is_match = ((tgt_lab[:, None] == prp_lab[None]).bool() & (ious > self.fg_thres).bool())

            # Proposal self IoU to filter non-overlap
            prp_self_iou = box_iou(prp_boxes, prp_boxes)  # [prp, prp]
            if self.require_overlap and (not self.use_gt_box):
                rel_possibility = ((prp_self_iou > 0).bool() & (prp_self_iou < 1).bool())
            else:
                num_prp = prp_boxes.shape[0]
                rel_possibility = torch.ones((num_prp, num_prp), device=device).long() - torch.eye(num_prp, device=device).long()
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility):
        ...
        for i in range(num_tgt_rels):
            ...
            fg_labels = torch.tensor(
                [tgt_rel_lab] * prp_tail_idxs.shape[0], dtype=torch.int64, device=device).view(-1, 1)
