...
def _get_gt_triplets(target):
    rel_pairs = _tensor_to_numpy(target.get("relation_tuple", np.zeros((0, 3), dtype=np.int64)))
    if rel_pairs.size == 0:
        return set()

    labels = _tensor_to_numpy(target.get("labels", np.zeros((0,), dtype=np.int64)))
    triplets = set()

    for row in rel_pairs:
        s_idx, o_idx, rel_label = map(int, row[:3])
        if s_idx < 0 or o_idx < 0 or s_idx >= len(labels) or o_idx >= len(labels):
            continue
        subj_cls = int(labels[s_idx])
        obj_cls = int(labels[o_idx])
        triplets.add((subj_cls, int(rel_label), obj_cls))

    return triplets

def _get_pred_triplets(pred):
    rel_pair_idxs = _tensor_to_numpy(pred.get("rel_pair_idxs", np.zeros((0, 2), dtype=np.int64)))
    pred_rel_labels = _tensor_to_numpy(pred.get("pred_rel_labels", np.zeros((0,), dtype=np.int64)))
    pred_obj_labels = _tensor_to_numpy(pred.get("pred_labels", np.zeros((0,), dtype=np.int64)))

    if len(rel_pair_idxs) == 0 or len(pred_rel_labels) == 0 or len(pred_obj_labels) == 0:
        return set()

    triplets = set()
    for (s_idx, o_idx), rel_label in zip(rel_pair_idxs, pred_rel_labels):
        s_idx = int(s_idx)
        o_idx = int(o_idx)
        rel_label = int(rel_label)
        if s_idx < 0 or o_idx < 0 or s_idx >= len(pred_obj_labels) or o_idx >= len(pred_obj_labels):
            continue
        subj_cls = int(pred_obj_labels[s_idx])
        obj_cls = int(pred_obj_labels[o_idx])
        triplets.add((subj_cls, rel_label, obj_cls))

    return triplets

def compute_image_success_metrics(dataset, predictions, n_eval, threshold=0.7):
    per_image = []
    success_count = 0
    valid_count = 0

    for i in range(n_eval):
        target = dataset.get_groundtruth(i, evaluation=True)
        gt_triplets = _get_gt_triplets(target)
        pred_triplets = _get_pred_triplets(predictions[i])

        gt_count = len(gt_triplets)
        pred_count = len(pred_triplets)
        correct_count = len(gt_triplets & pred_triplets)
        ...
        per_image.append({
            "image_index": i,
            "gt_triplets": gt_count,
            "pred_triplets": pred_count,
            "correct_triplets": correct_count,
            "image_triplet_recall": recall,
            "success": success,
        })

    image_success_rate = (success_count / valid_count) if valid_count > 0 else 0.0

    return {
        "threshold": threshold,
        "valid_image_count": valid_count,
        "successful_image_count": success_count,
...
def main():
    ...
    parser.add_argument(
        "--image-success-threshold",
        type=float,
        default=0.9,
        help="Count an image as successful if correct_triplets / gt_triplets >= this threshold (default: 0.7)"
    )
    ...
    logger.info("Running per-image triplet success evaluation ...")
    image_success = compute_image_success_metrics(
        dataset=dataset,
        predictions=predictions,
        n_eval=n_eval,
        threshold=args.image_success_threshold,
    )

    logger.info(
        f"Image success rate @ threshold {args.image_success_threshold:.2f}: "
        f"{image_success['successful_image_count']}/{image_success['valid_image_count']} "
        f"= {image_success['image_success_rate']:.4f}"
    )

    per_image_path = output_dir / "per_image_triplet_success.json"
    with open(per_image_path, "w") as f:
        json.dump(image_success, f, indent=2)

    logger.info(f"Per-image triplet success JSON saved to: {per_image_path}")
    ...
    summary["image_success_threshold"] = args.image_success_threshold
    summary["successful_image_count"] = image_success["successful_image_count"]
    summary["valid_image_count"] = image_success["valid_image_count"]
    summary["image_success_rate"] = round(image_success["image_success_rate"], 4)
