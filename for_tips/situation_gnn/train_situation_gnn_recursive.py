...
from dataset_path_resolver import load_records_from_tree, load_jsonl, read_class_list
from situation_gnn.dataset import build_graphs, class_weights_from_graphs, make_maps
...
def load_records_flexible(data_path: str | Path, jsonl_pattern: str = 'manual_labeled.jsonl'):
    ...
    if data_path.is_file():
        return load_jsonl(data_path), {
            ...
            'records_loaded': len(load_jsonl(data_path)),
        }
    records, discovery = load_records_from_tree(data_path, jsonl_pattern)
    discovery['mode'] = 'recursive_directory'
...
def split_indices(labels, seed=42, test_ratio=0.15, val_ratio=0.15):
    ...
    stratify2 = temp_labels if counts2 and len(counts2) > 1 and min(counts2.values()) >= 2 else None
...
def main():
    ...
    parser.add_argument('--data', type=str, required=True, help='Path to one jsonl file or a dataset root directory')
    parser.add_argument('--jsonl-pattern', type=str, default='manual_labeled.jsonl', help='Recursive filename match when --data is a directory')
    parser.add_argument('--outdir', type=str, default='./runs/situation_gnn')
    parser.add_argument('--task', type=str, default='multiclass', choices=['multiclass', 'binary', 'meaningful_multiclass'])
    ...
    records, discovery = load_records_flexible(args.data, args.jsonl_pattern)
    ...
    maps = make_maps(records, task=args.task, obj_classes=obj_classes, rel_classes=rel_classes)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save({'model_state': model.state_dict(), 'maps': maps, 'args': vars(args)}, best_path)
    ...
    split_info = {
        ...
        'val_idx': val_idx,
        'test_idx': test_idx,
        'train_scene_ids': [records[i].get('scene_id', f'sample_{i}') for i in train_idx],
        'val_scene_ids': [records[i].get('scene_id', f'sample_{i}') for i in val_idx],
        'test_scene_ids': [records[i].get('scene_id', f'sample_{i}') for i in test_idx],
        'discovery': discovery,
    }
    with open(outdir / 'splits.json', 'w', encoding='utf-8') as f:
        json.dump(split_info, f, ensure_ascii=False, indent=2)

    with open(outdir / 'data_discovery.json', 'w', encoding='utf-8') as f:
