    ...
    if mask.ndim == 3:
        if np.all(mask[..., 0] == mask[..., 1]) and np.all(mask[..., 1] == mask[..., 2]):
            ...
            raise ValueError(
                ...
                "This script expects a single-channel instance-id mask."
            )
    return mask
...
def make_label_lines(record, class_map):
    ...
    if mask.shape[0] != height or mask.shape[1] != width:
        raise ValueError(
            f"Mask/image size mismatch for {image_path}: "
    ...
    for i, obj in enumerate(record.get("objects", [])):
        ...
        if cls_name not in class_map:
            raise ValueError(
                f"Unknown class '{cls_name}' in scene {record.get('scene_id', 'unknown')}. "
                f"Please add it to FIXED_CLASS_NAMES."
            )
...
def write_yaml(root):
    yaml_text = []
    yaml_text.append(f"path: {root.resolve().as_posix()}")
    yaml_text.append("train: images/train")
    yaml_text.append("val: images/val")
    yaml_text.append("test: images/test")
    yaml_text.append("")
...
def process_split(records, split_name, class_map):
    ...
    for rec in records:
        ...
        dst_img = image_dir / src_img.name
        copy_or_link(src_img, dst_img)

        label_path = label_dir / f"{src_img.stem}.txt"
        ...
        if missing:
            print(f"[WARN] {src_img.name}: missing/empty masks for {missing}")
