...
FIXED_CLASS_NAMES.insert(0, "__background__")
FIXED_CLASS_NAMES_ENG = read_class_list(DATASET_ROOT / "object_classes_eng.txt")
...
FIXED_PREDICATE_NAMES = read_class_list(DATASET_ROOT / "relation_classes.txt")
...
def load_mask(mask_path):
    ...
    if mask.ndim == 3:
        if np.all(mask[..., 0] == mask[..., 1]) and np.all(mask[..., 1] == mask[..., 2]):
            ...
            raise ValueError(f"Mask at {mask_path} is multi-channel. Expected single-channel instance-id mask.")
    return mask
...
def make_annotation_for_object(obj, obj_idx, image_id, ann_id, class_map, mask):
    ...
    if cls_name not in class_map:
        raise ValueError(f"Unknown class '{cls_name}'. Add it to object_classes.txt.")
...
def convert_split(records, split_name, class_map, predicate_map):
    ...
    for rec in records:
        ...
        new_file_name = f"{stem}{src_img.suffix.lower()}"
        ...
        if INCLUDE_SEGMENTATION and mask_path:
            ...
            if mask_path.exists():
                ...
                if mask.shape[0] != height or mask.shape[1] != width:
                    raise ValueError(
                        f"Mask/image size mismatch for {src_img}: image=({width}, {height}), mask=({mask.shape[1]}, {mask.shape[0]})"
                    )
            else:
        ...
        images.append({
            ...
            "source_jsonl": rec.get("__meta__", {}).get("jsonl_path", ""),
        })
    ...
    coco = {
        ...
        "categories": [{"id": idx, "name": CLASS_NAME_MAP[name], "supercategory": "none"} for name, idx in class_map.items()],
        "rel_categories": [{"id": idx + 1, "name": name} for name, idx in predicate_map.items()],
...
def main():
    ...
    save_json([{"id": idx, "name": name, "supercategory": "none"} for name, idx in class_map.items()], OUTPUT_ROOT / "annotations" / "object_classes.json")
    save_json([{"id": idx, "name": name} for name, idx in predicate_map.items()], OUTPUT_ROOT / "annotations" / "predicate_classes.json")
